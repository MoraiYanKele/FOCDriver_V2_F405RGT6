#include "CmdTask.h"
#include "usart.h"

TaskHandle_t CmdTaskHandle = NULL;

CmdFrameTypedef cmdFrame; // 命令帧结构体
float receivedFloat = 0.0f;

// CRC8查找表（多项式：0x07，大小端无关，适用于各种MCU）
static const uint8_t crc8Table[256] = 
{
  
};

/**
 * @brief 使用查表法快速计算CRC8校验值
 * @param data 待校验的数据指针
 * @param length 数据长度
 * @return CRC8校验值
 * @note 使用多项式0x07(x^8 + x^2 + x^1 + x^0)，初始值0x00
 * @example 
 *   uint8_t data[] = {0x01, 0x02, 0x03};
 *   uint8_t crc = calculateCrc8(data, 3);
 */
uint8_t CalculateCrc8(const uint8_t* data, uint16_t length)
{
  uint8_t crc = CRC8_INIT_VALUE; // 初始值0x00
  
  // 参数检查
  if (data == NULL || length == 0)
  {
    return 0;
  }
  
  // 使用查表法计算CRC8
  for (uint16_t i = 0; i < length; i++)
  {
    crc = crc8Table[crc ^ data[i]];
  }
  
  return crc;
}

/**
 * @brief 验证命令帧的CRC8校验
 * @param frame 命令帧指针
 * @return true-校验通过，false-校验失败
 * @note 校验范围：从header到data字段（不包括crc8和footer）
 */
bool ValidateFrameCrc8(const CmdFrameTypedef* frame)
{
    if (frame == NULL)
    {
        return false;
    }
    
    // 检查帧头和帧尾
    if (frame->header != FRAME_HEADER || frame->footer != FRAME_FOOTER)
    {
        return false;
    }
    
    // 检查数据长度合法性
    if (frame->dataLength > sizeof(frame->data))
    {
        return false;
    }
    
    // 计算CRC8校验值（校验范围：header + cmd + dataLength + data）
    uint8_t calculatedCrc = CalculateCrc8((const uint8_t*)frame, 
                                         sizeof(frame->header) + 
                                         sizeof(frame->cmd) + 
                                         sizeof(frame->dataLength) + 
                                         frame->dataLength);
    
    return (calculatedCrc == frame->crc8);
}

/**
 * @brief 为命令帧生成CRC8校验值
 * @param frame 命令帧指针
 * @note 自动计算并设置frame->crc8字段
 */
void GenerateFrameCrc8(CmdFrameTypedef* frame)
{
    if (frame == NULL)
      return;
    
    // 设置帧头帧尾
    frame->header = FRAME_HEADER;
    frame->footer = FRAME_FOOTER;
    
    // 限制数据长度
    if (frame->dataLength > sizeof(frame->data))
      frame->dataLength = sizeof(frame->data);
    
    // 计算并设置CRC8校验值
    frame->crc8 = CalculateCrc8((const uint8_t*)frame, 
                               sizeof(frame->header) + 
                               sizeof(frame->cmd) + 
                               sizeof(frame->dataLength) + 
                               frame->dataLength);
}






/**
 * @brief 命令处理任务
 * @param argument 任务参数
 */
void CmdTask(void *argument)
{
  while (1)
  {    
    
    
    Delay(10);
  }
}

bool ReceiveCmdFrame(uint8_t *rxData, uint16_t length)
{
  if (length < sizeof(CmdFrameTypedef))
    return false;
  
  // 将接收到的数据复制到命令帧结构体
  memcpy(&cmdFrame, rxData, sizeof(CmdFrameTypedef));
  return true;
}


bool ProcessFloatCommand(const CmdFrameTypedef *frame)
{
  if (frame == NULL || frame->dataLength < sizeof(float))
    return false; // 无效的命令帧或数据长度不足
  memcpy(&receivedFloat, frame->data, sizeof(float));
  return true; // 成功处理命令
}

bool CheckCmdFrame(const CmdFrameTypedef *frame)
{
  if (cmdFrame.header == FRAME_HEADER && 
      cmdFrame.footer == FRAME_FOOTER &&
      ValidateFrameCrc8(frame))
    return true; // 命令帧有效
  
  else 
    return false; // 命令帧无效
}